...
function AdminOrderHistory({ currentUser, onBack, showNotification, theme, onEdit }) {
    ...
    return (
                        ...
                        {Object.entries(patientGroups).map(([patientKey, logsInGroup]) => (
                            ...
                            >
                                <div className="p-5 flex flex-col h-full">
                                    ...
                                    <div className="mt-auto pt-3 border-t border-slate-100 dark:border-slate-700/50 flex items-center justify-between text-[10px] font-bold text-slate-400">
                                        <span>คลิกเพื่อดูรายการสั่งยา</span>
                                        <span className="text-rose-500 font-black flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                                            ดูประวัติ <ArrowRight size={12} />
                                        </span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div key={log.id} className="bg-slate-50/50 dark:bg-slate-800/20 border border-slate-200 dark:border-slate-700/50 rounded-2xl overflow-hidden transition-all hover:border-slate-300 dark:hover:border-slate-600 shadow-sm hover:shadow">
                                        ...
                                        >
                                            <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
                                                <div>
                                                    <div className="text-[10px] text-slate-400 uppercase font-bold mb-1">เวลาสั่งยา</div>
                                                    <div className="text-sm font-bold">{new Date(log.timestamp).toLocaleTimeString('th-TH')}</div>
                                                </div>
                                                <div>
        ...
        {deleteConfirmId && (
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in" style={{ margin: 0 }}>
