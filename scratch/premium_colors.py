...
start_marker = '{/* Profile & Menu Container */}'
end_marker = '{/* Expandable Menu Container (Flows DOWNWARDS to avoid overlapping right buttons) */}'
end_full_marker = '                        </div>\n                    </div>'
...
# We want to replace the whole Profile & Menu Container up to its closing </div>
# The block ends before the Theme toggle button.
...
# Find the end of the container which is `</div>\n              )}` right before `idx_theme`
