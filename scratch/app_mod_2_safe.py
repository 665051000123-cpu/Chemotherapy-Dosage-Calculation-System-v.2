...
def safe_replace(content, search_text, insert_text):
...
content = safe_replace(content, "printerName: user.default_printer,", sticker_insert)
...
content = safe_replace(content, "printerName: user.working_formula_printer,", wf_insert)
...
content = safe_replace(content, "printerName: user.calculation_printer,", calc_insert)
...
content = safe_replace(content, "printerName: user.drug_info_printer,", drug_insert)
...
content = safe_replace(content, "printerName: user.all_drugs_printer,", all_drugs_insert)


with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)
print('Phase 2 safe replace done')
